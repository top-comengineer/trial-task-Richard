...
export default function Widget({
  ...
  const movingCss = `border-[2px] border-[#a10707] draggableHandle cursor-move`;

  const borderCss = `${
    isMoving
      ? movingCss
      : `border-[1px] border-[#789] ${tabSettings?.border ?? ""} ${tabSettings?.borderColor ?? ""}`
  }`;

  return (
    <div
      // border-2 border-gray-100 rounded-md
      className={`relative overflow-hidden overflow-y-auto ${borderCss} ${cn ?? ""}`}
    ...
    >
      <div
        onMouseOver={() => {
          ...
          setTimer(window.setTimeout(() => setMoverShowed(false), 3000));
        }}
      >
      ...
      {settingsShowed ? (
        ...
        <>
          ...
          <div className="p-2 capitalize">{widToName(wid) + " widget"}</div>
        </>
      ) : (
